from util.logger import get_main_logger, get_solver_logger
...
from solver.two_stage_solver import VRPSolver
...
if __name__ == "__main__":
    ...
    ds.visualize_scenarios(ds.clustered_scenarios[0])
    ...
    main_logger.info(f'Test for initial solution generation algorithm.')
    uav = config.UAV()
    model = config.Model()      # need dataclass instance
    demand_assignment_all, investment_decision = \
        init_sol.generate_initial_solution_greedy(demand_scenarios=ds,
                                                  parameters=param,
                                                  uav=uav, mod=model)
